...
/**
 * This class helps to change the skin of the IAutoFillTextBox Control
 *
 * @author Narayan G. Maharjan
 * @see <a href="http://www.blog.ngopal.com.np"> Blog </a>
 */
public class AutoFillTextBoxSkin<T>
		extends SkinBase<AutoFillTextBox<T>, AutoFillTextBoxBehavior<T>>
	...
	//This is the main Control of IAutoFillTextBox
	...
	public AutoFillTextBoxSkin(AutoFillTextBox text) {
						...
						if (cell.getItem() != null && cell.isFocused()) {
							...
							if (temporaryTxt.length() <= 0) {
								...
								if (listview.getItems().size() != data.size()) {
									temporaryTxt = textbox.getRawText();
								}
							}

							prev = temporaryTxt;
							textbox.rawTextProperty().removeListener(
									AutoFillTextBoxSkin.this);
							//textbox.setText(prev);
							textbox.rawTextProperty().setValue(
									cell.getItem().toString());
							textbox.rawTextProperty().addListener(
									AutoFillTextBoxSkin.this);
							textbox.selectRange(prev.length(),
									cell.getItem().toString().length());
							System.out.println(temporaryTxt.length() + "=" +
									textbox.getRawText().length() + "::" +
									cell.getItem().toString().length());
						}
					}
		...
		textbox.rawTextProperty().addListener(this);
	...
	public void showPopup() {
		...
		popup.show(
				...
				getWindow().getX() + textbox.localToScene(0, 0).getX() +
						textbox.getScene().getX(),
				getWindow().getY() + textbox.localToScene(0, 0).getY() +
	...
	public void changed(ObservableValue<? extends String> ov, String t,
		...
		if (ov.getValue().toString().length() > 0) {
			String txtdata = (textbox.getRawText()).trim();
